class CountLineChart {
	...
	addDataSet(processName) {
		...
		const dataset = {
		  ...
		  backgroundColor: colors[this.myChart.data.datasets.length % colors.length],
		  borderColor: colors[this.myChart.data.datasets.length % colors.length],
		  borderWidth: 1,
		  fill: true,
		  radius: 0,
		  pointHitRadius: 10,
...
class LengthLineChart {
	constructor(ctx){
		...
		this.myChart.canvas.style.height = '500px';
		this.LastMin = this.myChart.config.options.scales.x.min;
    	this.LastMax = this.myChart.config.options.scales.x.max;
	...
	fetchData() {
		...
		xhr.onprogress = () => { 
			...
			this.json.forEach((L) => {
				if (L['program'] != ""){
					metrics[L['program']] = Number(L['length']);
				}
			});
